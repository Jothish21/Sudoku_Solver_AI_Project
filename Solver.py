...
class SudokuSimulatedAnnealing(Annealer):
    ...
    def energy(self):
        # function to calculate the score for each column
        def column_score(n):
            # extracting values in the column
            column_values = [self.state[self.get_row_col(i, n)] for i in range(self.size)]
            unique_values = set(column_values) # getting uniw values in column
            score = -len(unique_values) # score is negative of the number of unique values
            return score

        # function to calculate the score for each row
        def row_score(n):
            # extracting values in the row
            row_values = [self.state[self.get_row_col(n, i)] for i in range(self.size)]
            unique_values = set(row_values) # getting uniw values in row
            score = -len(unique_values) # score is negative of the number of unique values
            return score
        ...
        for n in range(self.size):
            # adding the score for column and row to the total energy
            total_energy += column_score(n) + row_score(n)

        return total_energy

    def solve(self):
        # setting the parameters for simulated annealing
        self.Tmax = 15 # initial temperature
        self.Tmin = 10 # final temperature or cooling temp
        self.steps = 1000000 # number of steps 
        self.updates = 10000 # number of updates
